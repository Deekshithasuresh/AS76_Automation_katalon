...
int lymphocytesCountInt = 0

try {
	if (lymphocytesText == "-" ) {
		lymphocytesCountInt = 0
		WebUI.comment("ℹ️ Lymphocytes text is '${lymphocytesText}', treating it as 0")
	} else {
		lymphocytesCountInt = Integer.parseInt(lymphocytesText)
		WebUI.comment("✅ Parsed Lymphocytes count: ${lymphocytesCountInt}")
	}
} catch (NumberFormatException e) {
	WebUI.comment("❌ Failed to parse Lymphocytes count to int: ${e.message}")
}

CustomKeywords.'generic.Reclassification.classifyFromCellToCellMultiple'("Lymphocytes", "Basophils", lymphocytesCountInt)
