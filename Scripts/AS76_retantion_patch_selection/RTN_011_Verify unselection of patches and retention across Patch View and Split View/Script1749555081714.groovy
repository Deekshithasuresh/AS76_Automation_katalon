...
// DESELECT SECOND PATCH IN SPLIT VIEW
WebUI.comment("=== DESELECTING SECOND PATCH IN SPLIT VIEW ===")

if (selectedPatchesInfo.size() >= 2) {
	...
	try {
		...
		if (selectedPatches.size() >= 2) {
			// Get the second selected patch (index 1)
			WebElement secondSelectedPatch = selectedPatches.get(1)
			
			WebUI.comment("Attempting to deselect second patch by clicking on it again")
			
			// Scroll into view
			((org.openqa.selenium.JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", secondSelectedPatch)
			WebUI.delay(1)
			
			// Simply click on the already selected patch to deselect it
			secondSelectedPatch.click()
			WebUI.comment("✓ Deselected second patch by clicking on it")
			
			deselectionSuccessful = true
			WebUI.delay(2)
			
		} else {
			WebUI.comment("❌ Could not find enough selected patches for deselection")
		}
	...
	if (deselectionSuccessful) {
		...
		WebUI.comment("4. Switched back to patch view - " + (finalPatchViewCount == 2 ? "✅ 2 patches retained" : "❌ Incorrect count: " + finalPatchViewCount))
