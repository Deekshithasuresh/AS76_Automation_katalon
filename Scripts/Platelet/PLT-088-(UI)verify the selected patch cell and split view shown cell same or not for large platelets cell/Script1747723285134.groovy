...
// ✅ Step 2: If Large Platelets count is zero, skip patch logic
if (largePlateletCount == 0 || largePlateletRow == null) {
	KeywordUtil.markFailed("ℹ️ No Large Platelets found, skipping patch verification.")
	return
}

// ✅ Step 3: Click Large Platelets row to load patches
WebUI.executeJavaScript("arguments[0].scrollIntoView(true);", Arrays.asList(largePlateletRow))
WebUI.delay(1)
largePlateletRow.click()
WebUI.delay(2) // wait for patches to load

// ✅ Step 4: Locate Large Platelet patch thumbnails
WebDriver driver = DriverFactory.getWebDriver()
List<WebElement> patchThumbnails = driver.findElements(By.xpath("//div[contains(@class,'patch-img-container')]/img[1]"))
println("Total patches found for Large Platelets: " + patchThumbnails.size())

if (patchThumbnails.size() == 0) {
	println("⚠️ No patch thumbnails found for Large Platelets.")
	return
}
