...
//password masking method checked here.
// Define the TestObject for the password field
TestObject passwordField = findTestObject('View list of users/Page_Admin Console/input_Password_rbc-input-box')

// Verify that the password field is present
if (WebUI.verifyElementPresent(passwordField, 5)) {
    // Get the 'type' attribute of the password field
    String fieldType = WebUI.getAttribute(passwordField, 'type')

    // Check if the field type is 'password' (masked) or 'text' (unmasked)
    if (fieldType == 'password') {
        WebUI.comment('✅ Password is masked correctly.')
    } else if (fieldType == 'text') {
        WebUI.comment('❌ Password is not masked! It is displayed as plain text.')
    } else {
        WebUI.comment('❗ Unexpected field type: ' + fieldType)
    }
} else {
    WebUI.comment('❗ Password field not found.')
}
