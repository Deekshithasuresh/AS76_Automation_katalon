...
// Script to check the entered new password as text.
// Import necessary libraries
import com.kms.katalon.core.webui.keyword.WebUiBuiltInKeywords as WebUI
import com.kms.katalon.core.testobject.TestObject

// Locate the password input field
TestObject passwordField = findTestObject('Object Repository/View list of users/Page_Admin Console/input_Password_rbc-input-box')
...
if (WebUI.verifyElementPresent(passwordField, 5)) {
	...
	if (fieldType == 'text') {
		WebUI.comment("✅ The entered password is displayed as plain text.")
	} else if (fieldType == 'password') {
		WebUI.comment("❌ The password is masked (not displayed as text).")
	} else {
		WebUI.comment("⚠️ Unexpected input type: " + fieldType)
	}
} else {
	WebUI.comment("❗ Password field not found or not visible.")
}
