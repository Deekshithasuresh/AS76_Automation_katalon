 ...
 * If no "To be reviewed" reports are found, it assigns the 3rd report to deekshithaS.
 */
...
try {
	...
	if (toBeReviewedRows.size() > 0) {
		...
		if (currentAssignment == '') {
			...
			WebUI.comment('Report ' + reportId + ' is currently unassigned (To be reviewed). Assigning to deekshithaS.')
			...
			// Case 2: Report is already assigned to deekshithaS
			WebUI.comment('Report ' + reportId + ' is already assigned to deekshithaS.')
		} else {
			...
			// Use a dynamic TestObject for the re-assign button with the provided XPath
			TestObject reassignButton = new TestObject('Dynamic Re-assign Button')
			reassignButton.addProperty('xpath', ConditionType.EQUALS, '//div[@id=\'reassign-dialog\']//button[text()=\'Re-assign\']')
			
			WebUI.waitForElementPresent(reassignButton, 10)
	...
	// If no "To be reviewed" reports were processed, assign the 3rd report
	if (!processedToBeReviewed) {
		WebUI.comment('Proceeding to assign the 3rd report to deekshithaS')
		...
		if (allRows.size() >= 3) {
			...
			if (currentAssignment == '') {
				// Case 1: Report is unassigned
				WebUI.comment('Report ' + reportId + ' is currently unassigned (No reviewer). Assigning to deekshithaS.')
				WebUI.doubleClick(findTestObject('Object Repository/Summary/Page_PBS (1)/input_SIG0015_assigned_to'))
				WebUI.setText(findTestObject('Object Repository/Summary/Page_PBS (1)/input_SIG0015_assigned_to'), 'deekshithaS')
				WebUI.click(findTestObject('Object Repository/Summary/li_deekshithaS (2)'))
			} else if (currentAssignment == 'deekshithaS') {
