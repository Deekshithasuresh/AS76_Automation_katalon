...
/**
 * SUM_042_Verify the detected status for Large platelets and platelet clumps
 * when the Large platelets and platelet clump cells count is equal to 0
...
while ((!largePlateletsFound || !plateletClumpsFound) && (reportNumber < maxReports)) {
	...
	largePlateletsCountObject.addProperty('xpath', ConditionType.EQUALS, '//div[text()="Large Platelets"]/parent::div/div[@class="cell-count"]')
	...
	if (!largePlateletsFound && foundLargePlatelets) {
		...
		if (WebUI.waitForElementVisible(currentLargePlateletsCountObject, 5, FailureHandling.OPTIONAL)) {
			...
			try {
				...
				// MODIFIED: Check if count is EQUAL TO 0 (instead of greater than 0)
				if (largePlateletsCount == 0) {
					...
					WebUI.verifyMatch(largeDetectionStatus.trim(), "Not detected", true)  // Verify status is "Not detected"
					println("Report ID " + reportId + " (#" + reportNumber + "): Large Platelets count is 0 and status is correctly set to 'Not detected'")
					largePlateletsFound = true  // Mark Large Platelets as found
				} else {
					println("Report ID " + reportId + " (#" + reportNumber + "): Large Platelets count is " + largePlateletsCount + " (Detected)")
				}
		...
		if (WebUI.waitForElementVisible(currentPlateletClumpsCountObject, 5, FailureHandling.OPTIONAL)) {
			...
			try {
				...
				if (plateletClumpsCount == 0) {
					...
					WebUI.verifyMatch(clumpsDetectionStatus.trim(), "Not detected", true)  // Verify status is "Not detected"
					println("Report ID " + reportId + " (#" + reportNumber + "): Platelet Clumps count is 0 and status is correctly set to 'Not detected'")
					plateletClumpsFound = true  // Mark Platelet Clumps as found
				} else {
					println("Report ID " + reportId + " (#" + reportNumber + "): Platelet Clumps count is " + plateletClumpsCount + " (Detected)")
				}
			} catch (NumberFormatException e) {
				// Handle case where count text is not a valid number
...
if (largePlateletsFound) {
	...
	println("✓ Large Platelets with count = 0 found and status correctly displayed as 'Not detected'")
} else {
...
if (plateletClumpsFound) {
	...
	println("✓ Platelet Clumps with count = 0 found and status correctly displayed as 'Not detected'")
} else {
