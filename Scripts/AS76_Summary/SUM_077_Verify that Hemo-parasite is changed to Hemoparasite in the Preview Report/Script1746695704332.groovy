...
/**
 * Verify that "Hemo-parasite" is changed to "Hemoparasite" in the Preview Report
 * This function checks the text after clicking the approve button
 */
def verifyHemoparasiteTextInPreview() {
	// Expected text to be found
	...
	try {
		...
		if (previewText == expectedText) {
			...
			WebUI.markFailed(failMsg)
			return false
		}
	} catch (Exception e) {
		WebUI.comment("Failed to get text from preview: " + e.getMessage())
		WebUI.markFailedAndStop("Could not retrieve 'Hemoparasite' text from preview")
		return false
	}
}
...
try {
	...
	WebUI.markFailedAndStop("Test failed due to exception")
}
