...
try {
    ...
    if (toBeReviewedRows.size() > 0) {
        ...
        // Directly assign to chida using more reliable methods
        WebUI.comment(('Assigning report ' + reportId) + ' to chida')
        ...
        // Create a dynamic XPath to find the chida option in the dropdown
        TestObject chidaOption = new TestObject('chida Option')

        chidaOption.addProperty('xpath', ConditionType.EQUALS, '//li[text()=\'chida\' or contains(text(),\'chida\')]')

        // Make sure the dropdown option is visible before clicking
        WebUI.waitForElementVisible(chidaOption, 10)

        WebUI.click(chidaOption)
        ...
        if (!(WebUI.verifyElementAttributeValue(findTestObject('Object Repository/Summary/Page_PBS (1)/input_SIG0015_assigned_to'), 
            'value', 'chida', 5, FailureHandling.OPTIONAL))) {
            ...
            WebUI.setText(findTestObject('Object Repository/Summary/Page_PBS (1)/input_SIG0015_assigned_to'), 'chida')
    ...
    if (!(processedToBeReviewed)) {
        ...
        if (allRows.size() >= 3) {
            ...
            if (currentAssignment == 'chida') {
                // Case 1: Report is already assigned to chida
                WebUI.comment(('Report ' + reportId) + ' is already assigned to chida. No action needed.') // Case 2: Report is unassigned or assigned to someone else
                // Clear the current assignment and set to chida (following reference code)
                // Create and use dynamic TestObject for chida in dropdown to match the reference code
                ...
                WebUI.comment(((('Report ' + reportId) + ' is assigned to \'') + currentAssignment) + '\'. Reassigning to chida.')
...
//WebUI.verifyElementText(findTestObject('Object Repository/single_click/button_size_SPV'),'Size')

//WebUI.click(findTestObject('Object Repository/single_click/button_size_SPV'))
