...
try {
	...
	// Create the test object for single click on Platelet patch
	...
	if (patches.size() == 0) {
		println("✅ There are no cells present in Platelet Morphology")
	} else {
		// Perform single click to select the Platelet patch
		WebUI.click(targetElement)
		println("✅ Platelet Patch has been selected with single click")
		...
		println("✅ Platelet Patch selection is confirmed - found " + selectedPatches.size() + " selected patch(es)")
		} else {
		println("⚠️ No selected patch indicator found, but proceeding with outside click")
		}
		} catch (Exception e) {
		println("⚠️ Could not verify Platelet patch selection state: " + e.getMessage())
		}
		...
		List<WebElement> selectedPatches = driver.findElements(By.xpath("//div[contains(@class, 'selected-patch')]"))
		if (selectedPatches.size() == 0) {
		println("✅ Platelet Patch deselection is confirmed - no selected patches found")
		} else {
		println("⚠️ Found " + selectedPatches.size() + " selected patch(es) - patch may still be selected")
		}
		} catch (Exception e) {
		println("⚠️ Could not verify Platelet patch deselection state: " + e.getMessage())
		}
		...
		println("✅ Platelet Patch selection and outside-click deselection cycle completed successfully")
	}
	
} catch (Exception e) {
	println("❌ Failed to perform Platelet patch selection/deselection: " + e.getMessage())
