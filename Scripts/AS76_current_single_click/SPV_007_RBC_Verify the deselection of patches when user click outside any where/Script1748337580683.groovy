...
try {
	...
	// Create the test object for single click on RBC patch
	...
	if (patches.size() == 0) {
		println("✅ There are no cells present")
	} else {
		// Perform single click to select the RBC patch
		WebUI.click(targetElement)
		println("✅ RBC Patch has been selected with single click")
		...
		println("✅ RBC Patch selection is confirmed - found " + selectedPatches.size() + " selected patch(es)")
		} else {
		println("⚠️ No selected patch indicator found, but proceeding with outside click")
		}
		} catch (Exception e) {
		println("⚠️ Could not verify RBC patch selection state: " + e.getMessage())
		}
		...
		actions.moveByOffset(555, 87).click().perform() // Click at position (555, 87) from top-left
		...
		List<WebElement> selectedPatches = driver.findElements(By.xpath("//div[contains(@class, 'selected-patch')]"))
		if (selectedPatches.size() == 0) {
		println("✅ RBC Patch deselection is confirmed - no selected patches found")
		} else {
		println("⚠️ Found " + selectedPatches.size() + " selected patch(es) - patch may still be selected")
		}
		} catch (Exception e) {
		println("⚠️ Could not verify RBC patch deselection state: " + e.getMessage())
		}
		...
		println("✅ RBC Patch selection and outside-click deselection cycle completed successfully")
	}
	
	} catch (Exception e) {
	println("❌ Failed to perform RBC patch selection/deselection: " + e.getMessage())
...
try {
	...
	List<WebElement> patches = driver.findElements(By.xpath("//div[contains(@class, 'patch-img-container')]"))
	if (patches.size() == 0) {
		println("✅ There are no cells present in RBC COLOR")
	} else {
		// Perform single click to select the RBC patch
		WebUI.click(targetElement)
		println("✅ RBC Patch has been selected with single click")
...
try {
	...
	List<WebElement> patches = driver.findElements(By.xpath("//div[contains(@class, 'patch-img-container')]"))
	if (patches.size() == 0) {
		println("✅ There are no cells present in RBC INCLUSIONS")
	} else {
		// Perform single click to select the RBC patch
		WebUI.click(targetElement)
		println("✅ RBC Patch has been selected with single click")
