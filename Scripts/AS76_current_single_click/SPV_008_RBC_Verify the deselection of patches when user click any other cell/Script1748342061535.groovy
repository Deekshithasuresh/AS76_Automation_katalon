...
try {
    ...
    if (patches.size() == 0) {
        ...
        println('⚠️ Proceeding with navigation testing without patch selection') // Perform first single click to select the patch
...
try {
    ...
    if (patches.size() == 0) {
        ...
        println('⚠️ Proceeding with remaining testing without patch selection') // First click to select patch
        // Verify selection
