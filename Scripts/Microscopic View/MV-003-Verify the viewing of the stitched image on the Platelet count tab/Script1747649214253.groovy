...
// ---------- STEP: Click on the Count sub-tab ----------
TestObject countTab = new TestObject().addProperty(
    'xpath', ConditionType.EQUALS,
    "//button[@id='plateleteCountTab' and normalize-space()='Count']"
)
WebUI.waitForElementClickable(countTab, 10)
WebUI.click(countTab)
WebUI.comment("✔ Platelet Count sub-tab selected.")
...
WebUI.comment("✔ Microscopic view activated for Platelet Count.")

// ---------- STEP: Allow canvas to finish rendering ----------
WebUI.delay(3)

// ---------- STEP: Verify the stitched-image canvas is present ----------
TestObject pltCountCanvas = new TestObject().addProperty(
    'xpath', ConditionType.EQUALS,
    "//div[contains(@class,'ol-layer')]//canvas"
)
WebUI.waitForElementPresent(pltCountCanvas, 20)
WebUI.verifyElementVisible(pltCountCanvas)
WebUI.comment("✔ Stitched Platelet-Count microscopic image canvas is present and visible.")
