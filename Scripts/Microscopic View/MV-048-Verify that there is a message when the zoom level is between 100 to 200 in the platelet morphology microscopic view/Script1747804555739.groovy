...
TestObject morphTab = new TestObject().addProperty('xpath', ConditionType.EQUALS,
	"//button[@id='plateleteMorphologyTab' and contains(@class,'view-selection-btn')]"
)
...
WebUI.comment("✔ Microscopic view activated for Platelets Morphology.")
...
for (int i = 0; i < maxClicks; i++) {
    ...
    if (WebUI.verifyElementPresent(zoomWarning, 1, FailureHandling.OPTIONAL)) {
        WebUI.comment("✔ Zoom warning appeared after ${i + 1} clicks on Platelets Morphology view.")
        break
    }
}