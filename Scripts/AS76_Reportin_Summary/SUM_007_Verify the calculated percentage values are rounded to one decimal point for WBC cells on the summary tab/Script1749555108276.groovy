...
def validateDecimalFormat(String value) {
	...
	if (trimmedValue.equals('-')) {
		return [isValid: true, message: 'Value is dash - Valid format (no cell value detected)']
	}
	...
	try {
		...
		if (validationResult.isValid) {
			...
			if (cellValueText.equals('-')) {
				WebUI.comment("SUCCESS: ${cellName} value is dash (-) - Valid format (no cell value detected)")
			} else {
