...
WebUI.comment("✅ Browser resolution set to 1536x864")
...
if (allPatches.isEmpty()) {
	...
	if (firstRowCount == 12) {
		WebUI.comment("✅ Exactly 12 patches found in first row.")
	} else {
		WebUI.comment("❌ Expected 12 patches, but found ${firstRowCount} in first row.")
		KeywordUtil.markFailed('❌ Expected 12 patches, but found ${firstRowCount} in first row.')
