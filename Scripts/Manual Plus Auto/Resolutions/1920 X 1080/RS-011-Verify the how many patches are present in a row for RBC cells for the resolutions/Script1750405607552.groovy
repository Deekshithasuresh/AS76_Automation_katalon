...
WebUI.comment("✅ Browser resolution set to 1920x1080")
...
if (allPatches.isEmpty()) {
	...
	if (firstRowCount == 15) {
		WebUI.comment("✅ Exactly 15 patches found in first row.")
	} else {
		WebUI.comment("❌ Expected 15 patches, but found ${firstRowCount} in first row.")
		KeywordUtil.markFailed('❌ Expected 15 patches, but found ${firstRowCount} in first row.')
