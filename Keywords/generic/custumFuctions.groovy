...
public class custumFunctions {
	...
	def login() {
		...
		WebUI.setText(findTestObject('Object Repository/Platelet/Page_PBS/input_username_loginId'), 'pawanM')

		WebUI.setEncryptedText(findTestObject('Object Repository/Platelet/Page_PBS/input_password_loginPassword'), 'JBaPNhID5RC7zcsLVwaWIA==')

		WebUI.click(findTestObject('Object Repository/Platelet/Page_PBS/button_Sign In'))
	...
	def selectReportByStatus(String status) {
		...
		//WebUI.waitForElementVisible(findTestObject('Object Repository/Platelets/Page_PBS/th_Slide Id'), 15)
		...
		try {
			...
			KeywordUtil.markFailed(":x: Failed to find or click row with status '${status}': " + e.getMessage())
		}
	}
...
//		// 4) Find and click first row matching the status
//		try {
//			String xpath = "(//tr[.//td[contains(normalize-space(), '${status}')]])[2]"
//			TestObject matchingRow = new TestObject('dynamicReportRow')
//			matchingRow.addProperty('xpath', ConditionType.EQUALS, xpath)
//			WebUI.waitForElementClickable(matchingRow, 10)
//			WebElement rowElement = WebUiCommonHelper.findWebElement(matchingRow, 10)
...
//		}
//		catch (Exception e) {
//			String xpath = "(//tr[.//td[contains(normalize-space(), 'Under review')]])[5]"
//			TestObject matchingRow = new TestObject('dynamicReportRow')
//			matchingRow.addProperty('xpath', ConditionType.EQUALS, xpath)
//			WebUI.waitForElementClickable(matchingRow, 10)
//			WebElement rowElement = WebUiCommonHelper.findWebElement(matchingRow, 10)
...
//	}
	...
	def assignOrReassignOnTabs(String reviewerName, boolean confirmReassign = true) {
		...
		if (current != reviewerName) {
			...
			try {
				def popup = findTestObject('Object Repository/Platelet/Page_PBS/h2_Are you sure you want to re-assign this slide')

				def reassignBtn = findTestObject('Object Repository/Platelet/Page_PBS/button_Re-assign')

				def cancelBtn   = findTestObject('Object Repository/Platelet/Page_PBS/button_Cancel (1)')
...
def assignReviewerToReport(String initialStatus, String reviewerName) {
	...
	WebUI.scrollToElement(initialRow, 10)
	...
	if (before.equalsIgnoreCase("To be reviewed") && after.equalsIgnoreCase("Under review")) {
		WebUI.comment("✅ Status correctly changed from 'To be reviewed' to 'Under review'.")
	} else if (before.equalsIgnoreCase("Under review") && after.equalsIgnoreCase("Under review")) {
		WebUI.comment("✅ Status remained 'Under review' as expected.")
	} else {
		assert false : "❌ Unexpected status change. Before='${before}', After='${after}'"
	}
}
@Keyword
...
//
//	try {
//		// 1) Wait for the report table to appear
//		WebUI.waitForElementVisible(findTestObject('Object Repository/Page_PBS/th_Slide Id'), 15)
//		KeywordUtil.logInfo(":white_check_mark: Report table is visible.")
//
//		// 2) Scroll container to load all rows
//		String scrollSelector = "#reportListingTable > div > div"  // Update if needed
//		int scrollStep = 300
//		int delaySeconds = 1
//		int maxTries = 100
//		int tries = 0
...
//
//		while (tries < maxTries && !isBottom) {
//			// Check for the required status after every scroll step
//			try {
//				String xpath = "//tr[.//td[contains(normalize-space(), '${status}')]]"
//				TestObject matchingRow = new TestObject('dynamicReportRow')
//				matchingRow.addProperty('xpath', ConditionType.EQUALS, xpath)
//
//				// Use WebUiCommonHelper for reliable element detection
//				List<WebElement> elements = WebUiCommonHelper.findWebElements(matchingRow, 5)
//
//				if (!elements.isEmpty()) {
//					WebUI.comment(":white_check_mark: Found the status = '${status}' during scrolling.")
//					statusFound = true
//					// Click the first matching row
//					elements[0].click()
//					WebUI.comment(":white_check_mark: Clicked the first row with status = '${status}'")
//					break
//				}
//			} catch (Exception ex) {
//				KeywordUtil.logInfo(":warning: Status not found in the current view. Scrolling further.")
//			}
//
...
//			WebUI.delay(delaySeconds)
//
//			// Check if the bottom is reached or scrolling is stuck
//			if (scrollTop >= (scrollHeight - clientHeight - 10)) {
//				KeywordUtil.logInfo(":white_check_mark: Reached the bottom of the scroll container.")
//				isBottom = true
//			} else if (scrollTop == before) {
//				KeywordUtil.logInfo(":x: ScrollTop unchanged. Possibly stuck. Breaking.")
//				break
//			}
//			tries++
//		}
//
//		// 3) Handle the case when the status is not found
//		if (!statusFound) {
//			KeywordUtil.markWarning(":warning: Could not find any row with status '${status}' after scrolling.")
//		}
//	} catch (Exception e) {
//		KeywordUtil.markFailed(":x: An unexpected error occurred: " + e.getMessage())
//	} finally {
//		KeywordUtil.logInfo(":white_check_mark: Completed the selectReportByStatus keyword execution.")
//	}
//}
