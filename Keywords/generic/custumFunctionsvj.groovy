...
public class custumFunctionsvj {
	@Keyword
	def selectReportByStatus(String status) {
		...
		try {
			...
			KeywordUtil.markFailed("❌ Failed to find or click row with status '${status}': " + e.getMessage())
		}
	}
	@Keyword
	...
	def assignOrReassignOnTabs(String reviewerName, boolean confirmReassign = true) {
		...
		if (current != reviewerName) {
			...
			try {
				//def popup = findTestObject('Object Repository/Platelets/Page_PBS/h2_Are you sure you want to re-assign this slide')
				TestObject popup = new TestObject('dynamicReassignPopup')
				popup.addProperty('xpath', ConditionType.EQUALS, "//h2[contains(text(), 'Are you sure you want to re-assign')]")

				def reassignBtn = findTestObject('Object Repository/Platelets/Page_PBS/button_Re-assign')
				def cancelBtn   = findTestObject('Object Repository/Platelets/Page_PBS/button_Cancel')
				WebUI.delay(1) // optional
				if (WebUI.waitForElementPresent(popup, 3)) {
