...
public class Reclassificationvj {
	...
	def reclassifyAllWBCtoPlateletInBatches(String toPlateletCell, int batchSize = 5) {
		...
		try {
			...
			for (String fromWBCell : fromWBCells) {
				...
				WebElement wbcCellRow = driver.findElement(By.xpath("//table/tbody/tr/td[1][text()='" + fromWBCell + "']/parent::tr"))
	...
	//selected multiple patches and classify to wbc
	@Keyword
	...
	def reclassifyWBCToPlatelet(String fromCellName, String toPlateletCell, int patchCount) {
		...
		try {
			...
			if (fromInitialCount >= patchCount) {
				...
				WebElement wbcCellRow = driver.findElement(By.xpath("//table//td[text()='" + fromCellName + "']"))
				wbcCellRow.click()
	...
	def classifyToSubCell(String mainCell, String subCell) {
		...
		try {
			...
			WebElement mainRow = driver.findElement(By.xpath("//table/tbody/tr[td[1][text()='" + mainCell + "']]"))
			...
			WebElement subRow = driver.findElement(By.xpath("//table/tbody/tr[td[1][text()='" + subCell + "']]"))
	...
	// Reclassify platelet cells to  sub-cells of WBC
	@Keyword
	def classifyPlateletToWbcSubCell(String fromPlateletCell, String toWbcSubCell, int patchCount) {
		WebDriver driver = DriverFactory.getWebDriver()
		Actions actions = new Actions(driver)

		if (patchCount <= 0) {
			WebUI.comment("⏩ Skipping classification: 'patchCount' is 0 or less (${patchCount}).")
			return
		}

		String toMainWbcCell = getParentCellForSubCell(toWbcSubCell)
		...
		try {
			...
			int fromInitial = getCellCountInCurrentTab(driver, fromPlateletCell)

			WebUI.comment("Initial Platelet Count (${fromPlateletCell}): ${fromInitial}")

			if(fromInitial == 0) {
				KeywordUtil.markFailed("Initial Platelet Count (${fromPlateletCell}): ${fromInitial}, so skipping reclassification")
				return
			}

			// Step 2: Go to WBC tab to get initial sub-cell count
			WebUI.click(findTestObject('Object Repository/Platelets/Page_PBS/button_WBC'))
			WebUI.delay(1)
			int subInitial = getCellCountInCurrentTab(driver, toWbcSubCell)

			WebUI.comment("Initial WBC Sub-cell Count (${toWbcSubCell}): ${subInitial}")

			// Step 3: Return to Platelets tab and select the source cell row
			WebUI.click(findTestObject('Object Repository/Platelets/Page_PBS/button_Platelets'))
			WebUI.delay(1)
			WebUI.click(findTestObject('Object Repository/Platelets/Page_PBS/button_Morphology'))
			WebUI.delay(1)
			...
			if (patches.size() < patchCount) {
				WebUI.comment("⚠️ Only found ${patches.size()} patches. Cannot select ${patchCount}.")
				return
			}
			for (int i = 0; i < patchCount; i++) {
				patches.get(i).click()
			...
			// Step 5: Right-click and classify
			WebElement lastPatch = patches.get(patchCount - 1)
			actions.moveToElement(lastPatch).contextClick().perform()
			WebUI.comment("Right-clicked on selected patch(es)")

			WebElement classifyOption = driver.findElement(By.xpath("//span[text()='Classify']"))
			classifyOption.click()
			WebUI.comment("Clicked 'Classify'")

			// Step 6: Hover over main cell and select sub-cell
			WebElement mainCellElement = driver.findElement(By.xpath("//ul[contains(@class,'MuiMenu-list')]//li//div[text()='${toMainWbcCell}']"))
			actions.moveToElement(mainCellElement).perform()
			WebUI.comment("Hovered over main WBC cell: ${toMainWbcCell}")

			WebElement subCellElement = driver.findElement(By.xpath("(//ul[contains(@class,'MuiMenu-list')])[3]//li[contains(text(),'${toWbcSubCell}')]"))
			subCellElement.click()
			WebUI.comment("Selected WBC sub-cell: ${toWbcSubCell}")

			// Step 7: Verify classification message
			TestObject reclassifyMsg = new TestObject()
			reclassifyMsg.addProperty("xpath", ConditionType.EQUALS,"//div[contains(@class,'MuiSnackbarContent-message')]//div[contains(text(),'patches reclassified')]")
			WebUI.waitForElementVisible(reclassifyMsg, 5)
			boolean isPresent = WebUI.verifyElementPresent(reclassifyMsg, 5, FailureHandling.OPTIONAL)
			...
			// Step 8: Final counts
			int fromFinal = getCellCountInCurrentTab(driver, fromPlateletCell)
			WebUI.click(findTestObject('Object Repository/Platelets/Page_PBS/button_WBC'))
			WebUI.delay(1)
			int subFinal = getCellCountInCurrentTab(driver, toWbcSubCell)

			WebUI.comment("Final Platelet Count: ${fromFinal}, Final WBC Sub-cell Count: ${subFinal}")

			// Step 9: Assert count changes
			assert fromFinal == fromInitial - patchCount : "❌ Platelet count did not decrease by ${patchCount}"
			assert subFinal == subInitial + patchCount : "❌ WBC sub-cell count did not increase by ${patchCount}"

			WebUI.comment("✅ Reclassified ${patchCount} patch(es) from Platelet → ${toWbcSubCell}")
		} catch (Exception e) {
			WebUI.comment("❌ Classification error: ${e.message}")
			throw e
		}
	...
	// Reclassify platelet cells to main cells of WBC
	@Keyword
	def reclassifyPlateletToAllWBCMainCells(String fromPlateletCell, String toWbcCell, int patchCount) {
		WebDriver driver = DriverFactory.getWebDriver()
		Actions actions = new Actions(driver)
		...
		try {
			...
			if(fromInitialCount == 0) {
				KeywordUtil.markFailed("Initial Platelet Count (${fromPlateletCell}): ${fromInitialCount}, so skipping reclassification")
				return
			}
